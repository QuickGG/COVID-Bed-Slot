...
with open(r"C:\Users\Girish\Desktop\COVID_Bed_sllot\PROJECT\config.json",'r') as c:
...
def hosnav():
    ...
    return render_template("indexhos.html")
...
def slotbooking():
    ...
    if request.method=="POST":
        ...
        if not check2:
            flash("Hospital Code not exist","warning")
        srfiduser=Bookingpatients.query.filter_by(srfid=srfid).first()
        if  srfiduser:
            flash("You are only allowed to book one bed","warning")  
...
def available():
    query=db.engine.execute(f"SELECT * FROM `hospitaldata`")
    return render_template("availablebed.html",query=query)

@app.route("/hdetails",methods=['GET'])
@login_required
def hdetails():
    code=current_user.hcode
    data=Hospitaldata.query.filter_by(hcode=code).first()
    return render_template("hospitaldetails.html",data=data)
app.run(debug=True) 